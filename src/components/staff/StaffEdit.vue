            ...
            <div class="inputs" v-for="(title,index) in forming" :key="index">
                <div class="form">
                    ...
                    <input v-if="forming[index][0].type === 'ciryl'" maxlength="50"
                           ...
                           type="text" v-model="forming[index][0].name"
    ...
    export default {
        ...
        computed:{
            isSurnameValid () {
                ...
                let isSurnameValid = emailCodeRegex.test(this.forming[0][0].name)
                return isSurnameValid
            },
            isNameValid () {
                let emailCodeRegex = new RegExp(/^([А-ЯЁ]{1}[а-яё]{1,25}(-{0,1}[А-ЯЁ]{1}[а-яё]{1,25})?)$/)
                let isNameValid = emailCodeRegex.test(this.forming[1][0].name)
                return isNameValid
            },
            isFathernameValid () {
                let emailCodeRegex = new RegExp(/^([А-ЯЁ]{1}[а-яё]{1,49})$/)
                let isFathernameValid = emailCodeRegex.test(this.forming[2][0].name)
                return isFathernameValid
            },
            isPhoneValid () {
                // eslint-disable-next-line no-console
                console.log(this.forming[3][0].name)
                let emailCodeRegex = new RegExp(/(\+7|8)[- _]*\(?[- _]*(\d{3}[- _]*\)?([- _]*\d){7}|\d\d[- _]*\d\d[- _]*\)?([- _]*\d){6})/g)
                let isPhoneValid = emailCodeRegex.test(this.forming[3][0].name)
                return isPhoneValid
            },
            isEmailValid () {
                ...
                let isEmailValid = emailCodeRegex.test(this.forming[4][0].name)
                return isEmailValid
            },
            isPassportSeriaValid () {
                let emailCodeRegex = new RegExp(/^([0-9]{4})$/)
                let isPassportSeriaValid = emailCodeRegex.test(this.forming[7][0].name)
                return isPassportSeriaValid
            },
            isPassportNumValid () {
                let emailCodeRegex = new RegExp(/^([0-9]{6})$/)
                let isPassportNumValid = emailCodeRegex.test(this.forming[8][0].name)
                return isPassportNumValid
            },
            isPassportWhoValid () {
                // eslint-disable-next-line no-useless-escape
                let emailCodeRegex = new RegExp(/^([А-Яа-я]№?.?,?-? ?)+[а-я]{1}$/)
                let isPassportWhoValid = emailCodeRegex.test(this.forming[10][0].name)
                return isPassportWhoValid
            },
            isLoginValid () {
                let emailCodeRegex = new RegExp(/^[A-Za-z0-9_-]{2,16}$/)
                let isLoginValid = emailCodeRegex.test(this.forming[11][0].name)
                return isLoginValid
            },
            isPasswordValid () {
                let emailCodeRegex = new RegExp(/^[A-Za-z0-9_-]{8,16}$/)
                let isPasswordValid = emailCodeRegex.test(this.forming[12][0].name)
            ...
            getUser (){
                axios({
                    ...
                    .then((response) => {
                        ...
                        this.forming[2][0].name = response.data.fathername
                        this.forming[3][0].name = response.data.telephone.replace(/(\w+)/g, '+7$1')
                        this.forming[4][0].name = response.data.email
                        this.forming[5][0].name = response.data.birth_date
                        this.forming[6][0].name = response.data.date_of_employment
                        this.forming[7][0].name = response.data.passport_series
                        this.forming[8][0].name = response.data.passport_number
                    ...
                    if(this.forming[i][0].name == null || this.forming[i][0].name == '' || this.forming[i][0].name == undefined) {
                        ...
                        if (this.forming[i][0].fieldName === 'surname'){
                            if (this.isSurnameValid === false){
                                ...
                                this.counter++
                                this.user.surname = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'name'){
                            if (this.isNameValid === false){
                                ...
                                this.counter++
                                this.user.name = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'fathername'){
                            if (this.isFathernameValid === false){
                                ...
                                this.counter++
                                this.user.fathername = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'phone'){
                            if (this.isPhoneValid === false){
                                ...
                                this.counter++
                                if(this.data.telephone === this.forming[i][0].name){
                                    this.user.telephone = this.forming[i][0].name
                                        .replace(/\+/g, "" )
                                        .replace(/7/g, "")
                                }
                                else{
                        ...
                        if (this.forming[i][0].fieldName === 'email'){
                            if (this.isEmailValid === false){
                                ...
                                this.counter++
                                this.user.email = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'passportseria'){
                            if (this.isPassportSeriaValid === false){
                                ...
                                this.counter++
                                this.user.passport_series = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'passportnumber'){
                            if (this.isPassportNumValid === false){
                                ...
                                this.counter++
                                this.user.passport_number = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'passportwho'){
                            if (this.isPassportWhoValid === false){
                                ...
                                this.counter++
                                this.user.passport_issued_by = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'login'){
                            if(this.forming[11][0].name.length < 2){
                                this.forming[i][0].errorIndex = 4
                                this.forming[i][0].borderError = true
                            }
                            else if(this.forming[11][0].name.length > 16){
                                this.forming[i][0].errorIndex = 4
                                this.forming[i][0].borderError = true
                            }
                                ...
                                this.counter++
                                this.user.login = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].fieldName === 'password'){
                            if(this.forming[12][0].name.length < 8){
                                this.forming[i][0].errorIndex = 3
                                this.forming[i][0].borderError = true
                            }
                            else if(this.forming[12][0].name.length > 16){
                                this.forming[i][0].errorIndex = 3
                                this.forming[i][0].borderError = true
                            }
                                ...
                                this.counter++
                                this.user.pass = this.forming[i][0].name
                            }
                        }
                        if (this.forming[i][0].type === 'date'){
                            let today = new Date()
                            let ourDate = this.forming[i][0].name
                            ...
                            if (ourDateparse.setHours(0,0,0,0) > today.setHours(0,0,0,0)){
                                ...
                                this.counter++
                                if(this.data.date_of_issue_of_passport === this.forming[9][0].name){
                                    this.user.date_of_issue_of_passport = this.forming[9][0].name
                                }
                                else{
                                    this.user.date_of_issue_of_passport = this.forming[9][0].name.replace( /-/g, "." )
                                        .replace(/(\w+).(\w+).(\w+)/i, '$3.$2.$1')
                                }
                                if(this.data.birth_date === this.forming[5][0].name){
                                    this.user.birth_date = this.forming[5][0].name
                                }
                                else{
                                    this.user.birth_date = this.forming[5][0].name.replace( /-/g, "." )
                                        .replace(/(\w+).(\w+).(\w+)/i, '$3.$2.$1')
                                }
                                if(this.data.date_of_employment === this.forming[6][0].name){
                                    this.user.date_of_employment = this.forming[6][0].name
                                }
                                else{
                                    this.user.date_of_employment = this.forming[6][0].name.replace( /-/g, "." )
                                        .replace(/(\w+).(\w+).(\w+)/i, '$3.$2.$1')
                                }
                            }
                        }
