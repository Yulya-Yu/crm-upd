...
const actions = {
    async fetchStaff({ commit }) {
        const response = await axios.get('http://api.catering.student.smartworld.team:2280/employee/', { auth: { username: 'admin', password: 'dj5ghg67' } });
        commit('setStaff', response.data.items);
    },
    async deleteStaff({ commit }, id) {
        await axios.post(`http://api.catering.student.smartworld.team:2280/employee/dismiss?id=${id}`, { username: 'admin' }, { auth: { username: 'admin', password: 'dj5ghg67', } });
        commit('deleteStaff', id);
    },
    async fetchMenu({ commit }) {
        const response = await axios.get('http://api.catering.student.smartworld.team:2280/category/', { auth: { username: 'admin', password: 'dj5ghg67' } });
    ...
    async fetchDeletedStaff({ commit }) {
        const response = await axios.get('http://api.catering.student.smartworld.team:2280/employee/dismissed-staff', { auth: { username: 'admin', password: 'dj5ghg67' } });
        commit('setDeletedStaff', response.data.items);
    },
    async restoreStaff({ commit }, id) {
        await axios.post(`http://api.catering.student.smartworld.team:2280/employee/?id=${id}`, { username: 'admin' }, { auth: { username: 'admin', password: 'dj5ghg67', } });
